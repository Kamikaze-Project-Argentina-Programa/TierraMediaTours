...
public class UserDAOImpl implements UserDAO { 
 ...
 public int insertR(User user) { 
	  try { 
	   String sql = "INSERT INTO users (name, password, admin, money, time, preferences) VALUES (?, ?, ?, ?, ?, ?)"; 
	   Connection conn = ConnectionProvider.getConnection(); 
 ...
 public int update(User user) { 
  try { 
   String sql = "UPDATE users SET money = ?, time = ? WHERE id = ?"; 
   Connection conn = ConnectionProvider.getConnection(); 
 ...
 private User toUser(ResultSet userRegister) throws SQLException { 
  return new User(userRegister.getInt(1), userRegister.getString(2), userRegister.getString(3), 
    userRegister.getInt(4), userRegister.getInt(5), userRegister.getDouble(6), userRegister.getInt(7)); 
 } 
